            ...
            <section className="py-16 bg-transparent">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="text-center mb-12">
                        ...
                        <p className="text-lg" style={{ color: 'var(--color-text-muted)' }}>Загрузка новостей...</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {[...Array(limit)].map((_, i) => (
                            <div key={i} className="bg-transparent rounded-2xl overflow-hidden shadow-lg animate-pulse">
                                ...
                                <div className="p-6">
                                    <div className="h-4 bg-surface rounded mb-3 w-1/3" />
                                    <div className="h-6 bg-surface rounded mb-2" />
                                    <div className="h-4 bg-surface rounded mb-4 w-2/3" />
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        <section className="py-16 bg-transparent">
            <div className="max-w-7xl mx-auto px-4">
                ...
                <div className="flex items-center justify-between mb-12">
                    <div>
                        <h2 className="text-3xl md:text-4xl font-bold mb-2" style={{ color: 'var(--color-primary-dark)' }}>
                            📰 Последние новости
                        </h2>
                    ...
                    {news.map((item, index) => {
                        ...
                        return (
                            <Link
                                key={item._id || index}
                                href={`/news/${item.slug}`}
                                className="group bg-bg rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2"
                            >
                                {/* Изображение */}
                                ...
                                <div className="p-6">
                                    ...
                                    {/* Заголовок */}
                                    <h3 className="text-lg font-bold mb-3 line-clamp-2 group-hover:text-primary transition-colors" style={{ color: 'var(--color-primary-dark)' }}>
                                        {item.title}
                                    </h3>
                                    ...
                                    {/* Читать далее */}
                                    <div className="flex items-center gap-2 font-semibold text-sm group-hover:gap-3 transition-all" style={{ color: 'var(--color-primary)' }}>
                                        Читать далее
