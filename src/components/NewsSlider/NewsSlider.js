...
export default function NewsSlider({ limit = 5, autoPlay = true, interval = 8000 }) {
                ...
                if (isMounted && data?.success && data.data?.length > 0) {
                    const processedNews = data.data.map(item => ({
                        ...
                        media: getFirstMedia(item.contentBlocks) || (item.featuredImage ? {
                            type: 'image', url: item.featuredImage, alt: item.title
                        } : null)
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => setCurrent(prev => (prev + 1) % news.length), interval);
        return () => clearInterval(timer);
    }, [isAutoPlay, isHovered, news.length, interval]);

    const next = useCallback(() => setCurrent(prev => (prev + 1) % news.length), [news.length]);
    const prev = useCallback(() => setCurrent(prev => (prev - 1 + news.length) % news.length), [news.length]);
    const goToSlide = (index) => setCurrent(index);

    const handleTouchStart = (e) => { touchStartX.current = e.touches[0].clientX; setIsAutoPlay(false); };
    ...
    return (
        <section className={styles.newsSlider} ref={sliderRef} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}
            onMouseEnter={() => { setIsHovered(true); setIsAutoPlay(false); }}
            onMouseLeave={() => { setIsHovered(false); setIsAutoPlay(autoPlay); }}
            aria-label="новости">

            <button className={`${styles.sliderArrow} ${styles.prev}`} onClick={prev} aria-label="Предыдущая новость" type="button">
            ...
            <div className={styles.sliderViewport}>
                <div className={styles.sliderTrack} style={{ transform: `translateX(-${current * 100}%)` }}>
                    {news.map((item, index) => {
                        ...
                        return (
                            <article key={item._id || index} className={`${styles.slide} ${isActive ? styles.active : ''}`}>
                                <div className={styles.mediaContainer}>
                                    {media?.type === 'youtube' ? (
                                        <div className={styles.videoWrapper}>
                                            <iframe src={`https://www.youtube.com/embed/${media.url}?rel=0&modestbranding=1`}
                                                title={item.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                                allowFullScreen loading={isActive ? 'eager' : 'lazy'} className={styles.videoFrame} />
                                        ...
                                        <video src={media.url} className={styles.slideImage} muted playsInline loop autoPlay={isActive} preload="metadata" style={{ objectFit: 'cover' }} />
                                    ) : media?.type === 'image' ? (
                                        <Image src={media.url} alt={media.alt || item.title} className={styles.slideImage} width={1200} height={600}
                                            priority={isActive} quality={90} sizes="(max-width: 768px) 100vw, 800px" />
                                    ) : (
                                ...
                                <Link href={newsUrl} className={styles.slideContent}>
                                    <div className={styles.contentInner}>
                                        ...
                                        {item.excerpt && <p className={styles.slideExcerpt}>{item.excerpt}</p>}
                                        <footer className={styles.slideFooter}>
                                            {item.publishedAt && (
                                                <time className={styles.slideDate} dateTime={item.publishedAt}>
            ...
            <button className={`${styles.sliderArrow} ${styles.next}`} onClick={next} aria-label="Следующая новость" type="button">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            ...
            <div className={styles.sliderIndicators} role="tablist" aria-label="Выбор новости">
                {news.map((_, index) => (
                    <button key={index} className={`${styles.indicator} ${index === current ? styles.active : ''}`}
                        onClick={() => goToSlide(index)} role="tab" aria-selected={index === current} aria-label={`Новость ${index + 1} из ${news.length}`} type="button">
            ...
            <div className={styles.slideCounter} aria-hidden="true">
                <span>{String(current + 1).padStart(2, '0')}</span><span>/</span><span>{String(news.length).padStart(2, '0')}</span>
            </div>
