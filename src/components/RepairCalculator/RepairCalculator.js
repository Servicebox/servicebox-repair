    ...
    const getFilteredServices = () => {
        ...
        return Object.entries(category.services).filter(([key, svc]) => {
            ...
            if (svc.portType) {
                ...
                return false; // Если модель не указывает порт, услуга не подходит
            }
            // 3. Фильтрация по флагам модели
            ...
            // 4. Услуги, которые не существуют у некоторых брендов (например, Face ID у Samsung)
            ...
            if (key === 'glass' && brand !== 'apple') return false; // Переклейка стекла — только для Apple
            // 5. Услуги, зависящие от конкретных моделей (уже включены в фильтрацию по флагам)
            // liquid_metal, ssd_upgrade — уже фильтруются по modelData.id
            return true;
        });
    };
