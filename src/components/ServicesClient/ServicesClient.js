...
export default function ServicesClient({ initialCategories }) {
    ...
    if (!initialCategories.length) {
        return (
            <div className="text-center py-12 bg-bg rounded-2xl shadow-sm max-w-7xl mx-auto px-4">
                <p className="text-muted">Не удалось загрузить список услуг.</p>
            </div>
        );
    }
        ...
        <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="mb-8">
                <input
                    ...
                    className="w-full max-w-xl mx-auto block px-4 py-3 border border-border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-primary outline-none transition-shadow"
                />
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredCategories.map((category) => (
                        <Link
                            ...
                            className="group block bg-bg rounded-2xl shadow-lg border border-border p-8 hover:shadow-2xl hover:border-primary hover:-translate-y-2 transition-all duration-300 cursor-pointer"
                        >
                            <div className="flex flex-col items-center text-center">
                                ...
                                <div className="flex items-center justify-between w-full mt-auto">
                                    ...
                                    <span className="text-primary font-semibold group-hover:text-primaryDark transition-colors">
                                        Подробнее →
                                    </span>
                                </div>
                            </div>
